...
func GetLoanSearchResult(login_branch string, login_user string, sessionId string, user_lang string, domain string) *[]int {
	...
	page = *ProcessLoanSearchResult(string(content))

	for _, id := range page {
		fmt.Println("id  Error reading response body:", id)
		// targeturl := "https://" + domain + "/nobility_asset_limited/~/search_loan.php?type=view&do=inquire&state=search&page=" + string(id)
		...
		loanIds := *ProcessLoanSearchResultByPage(string(content))
