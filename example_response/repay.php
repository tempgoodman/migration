...
/*------------------------------
 ...
 |------------------------------*/
var button_text = '';
var selected_repay_date = '0000-00-00';
...
$(function() {
	...
	$('#main-form').submit(function() {
		...
		$('#' + button_text + '-button').button('loading');
		$.ajax({
			type: 'POST',
			global: true,
			...
			success: function(response) {
					...
					form_response(response, $('#' + button_text + '-button'));
				},
			error: function() {
					$('#' + button_text + '-button').button('reset');
				}
		});
		return false;
	});
	...
	/*------------------------------
	 |	form validation
	 |------------------------------*/
	void2_form_validator = $('#void2-form').validate({
		rules: {
			'void2_date': {
					dgt_repay_date: function() { return selected_repay_date; }
				}
		},
		messages: {
			'void2_date': {
					dgt_repay_date: $.validator.format("日期必須大於還款日期 ({0})")
				}
		}
	});
	
	
	// date greater than (repay date)
	$.validator.addMethod('dgt_repay_date', function(value, element, param) {
		return this.optional(element) || date_diff(param, value) >= 0;
	}, $.validator.format("This date must be or later than {0}"));
	...
	$('#void2-form').submit(function() {
		...
		// create data string
		var param = $('#main-form, #void2-form').find('input, textarea, select').serialize();
		var data = {
		};
		param += '&' + $.param(data);
...
/*------------------------------
 |	repay functions
 |------------------------------*/
function select_repay(element) {
	if (!element.hasClass('selected')) {
		$('#repay-list .selected').find('i[class^=icon-]').removeClass('icon-white');
		$('#repay-list .selected').removeClass('selected');
		
		// enable buttons
		if (element.find('[id$="_migrate_lock"]').val() == 1) {
			$('#repay-edit-button, #repay-delete-button, #repay-void-button, #repay-void2-button, #repay-print-button').prop('disabled', true);
		}
		else if (element.find('[id$="_repay_type"]').val() == 'autopay_temp' && element.nextAll('.repay_row').find('[id$="_autopay_status"][value="pending"]').length == element.nextAll('.repay_row').length) {
			$('#repay-edit-button').prop('disabled', false);
			$('#repay-delete-button').prop('disabled', false);
			$('#repay-void-button').prop('disabled', false);
			$('#repay-void2-button').prop('disabled', true);
		}
		else if (element.nextAll('.repay_row').find('[id$="_void"][value="1"]').length < element.nextAll('.repay_row').length || false || false) {
			$('#repay-edit-button, #repay-delete-button, #repay-void-button, #repay-void2-button').prop('disabled', true);
		}
		else if (element.find('[id$="_void"]').val() == 1) {
			$('#repay-edit-button, #repay-void-button, #repay-void2-button, #repay-print-button').prop('disabled', true);
			$('#repay-delete-button').prop('disabled', false);
		}
		else if (element.find('[id$="_void2"]').val() == 1) {
			$('#repay-edit-button, #repay-void-button, #repay-void2-button, #repay-print-button').prop('disabled', true);
			$('#repay-delete-button').prop('disabled', false);
		}
		else if (element.find('[id$="_autopay_id"]').val() != 0) {
			$('#repay-edit-button, #repay-void-button, #repay-void2-button, #repay-print-button').prop('disabled', true);
			$('#repay-delete-button').prop('disabled', false);
		}
		else if (element.find('[id$="_repay_type"]').val() == 'withdraw_temp') {
			$('#repay-edit-button').prop('disabled', false);
			$('#repay-delete-button').prop('disabled', false);
			$('#repay-void-button').prop('disabled', false);
			$('#repay-void2-button').prop('disabled', true);
			...
			$('#repay-edit-button').prop('disabled', (element.find('[name$="_repay_type"]').val() != 'normal'));
			$('#repay-delete-button').prop('disabled', false);
			$('#repay-void-button').prop('disabled', false);
			$('#repay-void2-button').prop('disabled', false);
		}
		
		if (element.find('[id$="_void"]').val() == 1 || element.find('[id$="_void2"]').val() == 1) {
			$('#repay-print-button').prop('disabled', true);
		}
		else {
			$('#repay-print-button').prop('disabled', false);
		}
		
				// enable repay-edit-button
		if ($('#repay-edit-button').prop('disabled')) {
			if (!(element.find('[id$="_migrate_lock"]').val() == 1 || element.find('[id$="_autopay_id"]').val() != 0 || element.find('[id$="_void"]').val() == 1 || element.find('[id$="_void2"]').val() == 1)) {
				$('#repay-edit-button').prop('disabled', false);
				
				// use type=edit_detail
				$('#repay-edit-button').attr('onclick', $('#repay-edit-button').attr('onclick').replace("open_repay_detail('edit')", "open_repay_detail('edit_detail')"));
			}
		}
		else {
			$('#repay-edit-button').attr('onclick', $('#repay-edit-button').attr('onclick').replace("open_repay_detail('edit_detail')", "open_repay_detail('edit')"));
		}
				
		
		selected_repay_date = element.find('[id$="_repay_date"]').val();
		selected_repay_id = element.find('[id$="_repay_id"]').val();
		element.find('[name=select_choice]').prop('checked', true);
		element.find('i[class^=icon-]').addClass('icon-white');
		element.addClass('selected');
		
		$('#repay-print-button').unbind('click').click(function() {
			new_window('print_docs_repay_receipt.php?type=view&id=' + selected_repay_id, 750, 800, 'print_docs_repay_receipt');
		});
	}
	else {	
		selected_repay_date = '0000-00-00';
		element.find('[name=select_choice]').prop('checked', false);
		$('#repay-list .selected').find('i[class^=icon-]').removeClass('icon-white');
		$('#repay-list .selected').removeClass('selected');
...
function open_repay_detail(type, repay_id) {
	if (typeof repay_id == 'undefined') {
		repay_id = $('#' + $('#repay-list .selected').attr('id') + '_repay_id').val();
	}
	new_window('repay_detail.php?type=' + type + '&loan_id=125&id=' + repay_id + '&menu=', 1100, 650, 'repay_detail');
	return;
}


function check_autopay_complete(element) {
	var is_valid = ($('.repay_row [id$="autopay_status"][value="pending"]').length == 0);
	
	if (!is_valid) {
		new_window('repay_detail.php?type=add&loan_id=125&autopay_temp=1', 1100, 650, 'repay_detail');
	}
	...
	<div class="subnav-inner">
		<div class="container">
			...
			<ul class="nav2">
				...
				<li><button class="btn  search-box " href="search_loan.php?type=view&do=repay" id="search-button" type="button"  onClick="return false;"  title="進階搜尋">進階搜尋</button></li>
			</ul><!--/search buttons-->
						
			<!--action buttons-->
			<ul class="nav2 pull-right">
				<li><button class="btn" type="button"  onClick="if (!check_autopay_complete(this)) return; open_repay_detail('add', '');"  title="新增" accesskey="N"><i class="icon-file"></i> 新增</button></li>
				<li><button id="repay-edit-button" class="btn" type="button"  onClick="if (!check_active_period(this, selected_repay_date)) return; open_repay_detail('edit');"  disabled title="修改" accesskey="E"><i class="icon-edit"></i> 修改</button></li>
				<li><button id="repay-void2-button" class="btn" type="button"  onClick="button_text = 'void2'; $('#void2-modal').modal('show');"  disabled title="退款/票" accesskey="W"><i class="icon-ban-circle"></i> 退款/票</button></li>
				<li><button id="repay-delete-button" class="btn btn-danger" type="button"  onClick="if (!check_active_period(this, selected_repay_date)) return; button_text = 'delete'; $('#delete-modal').modal('show');"  disabled title="刪除" accesskey="D"><i class="icon-trash icon-white"></i> 刪除</button></li>
				<li><button id="repay-void-button" class="btn btn-warning" type="button"  onClick="if (!check_active_period(this, selected_repay_date)) return; button_text = 'void'; $('#void-modal').modal('show');"  disabled title="Void" accesskey="V"><i class="icon-lock icon-white"></i> Void</button></li>
				<li><button id="repay-print-button" class="btn" type="button" disabled title="列印收據"><i class="icon-print"></i> 列印收據</button></li>
			</ul><!--/action buttons-->
		</div>
	</div>
</div>
...
<div class="container">
	<div class="page-header" style="position: relative;">
		<h1>貸款 &raquo; 還款處理</h1>
			...
			<button class="btn" type="button"  onClick="refresh('loan_payout.php?id=125');"  title="放款">放款</button>
			<button class="btn btn-inverse" type="button"  title="還款">還款</button>
				...
				<div class="span12">
							...
							<div class="control-group" id="passport-control">
								...
								<div class="span12">
									<div class="control-group">
										...
										<div class="controls">
											<button class="btn" type="button" onClick="new_window('loan_info.php?type=view&id=125&menu=no', 1220, 650, 'loan_info');" title="查看貸款資料">查看貸款資料</button>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="tabbable">
						<ul class="nav nav-tabs">
							<li class="active"><a href="#tab-repay-detail-information" data-toggle="tab">還款資料</a></li>
							<li><a href="#tab-repay-condition-information" data-toggle="tab">還款狀況</a></li>
						</ul>
					
						<div class="tab-content container-fluid">
							<div class="tab-pane active" id="tab-repay-detail-information">
								<table class="table table-striped" style="table-layout: fixed;" border="0">
									<colgroup>
										<col width="4%" />
										<col width="7%" />
										<col width="7%" />
									...
									<thead>
										<tr>
											...
											<th>還款編號</th>
											<th>還款類別</th>
											<th style="text-align: center;">供款期數</th>
											<th style="text-align: right;">還款金額</th>
											<th style="text-align: right;">過期利息</th>
											<th style="text-align: right;">行政費</th>
											<th style="text-align: right;">收取金額</th>
											<th style="text-align: right;">暫存金額</th>
											<th style="text-align: center;">備註</th>
											<th>&nbsp;</th>
										</tr>
									</thead>
									
									<tbody id="repay-list" style="table-layout: fixed;">
																																																		<tr class="repay_row" id="repay_0" onClick="select_repay($(this));" style="cursor: pointer; ">
											<td style="text-align: center;"><input class="select_choice" name="select_choice" type="radio" value="0" /></td>
											<td>2024-02-08</td>
											<td>R2402026</td>
											<td>還款</td>
											...
											<td style="text-align: center;"><button class="btn btn-small" type="button" onClick="if ($.browser.msie) { window.event.cancelBubble = true; } else { event.stopPropagation(); } open_repay_detail('view', '400');">詳細</button>
												<input id="repay_0_void" name="repay_0_void" type="hidden" value="" />
												<input id="repay_0_void2" name="repay_0_void2" type="hidden" value="" />
												<input id="repay_0_repay_id" name="repay_0_repay_id" type="hidden" value="400" />
												<input id="repay_0_autopay_id" name="repay_0_autopay_id" type="hidden" value="0" />
												<input id="repay_0_autopay_no" name="repay_0_autopay_no" type="hidden" value="" />
												<input id="repay_0_autopay_status" name="repay_0_autopay_status" type="hidden" value="" />
												<input id="repay_0_repay_date" name="repay_0_repay_date" type="hidden" value="2024-02-08" />
												<input id="repay_0_repay_no" name="repay_0_repay_no" type="hidden" value="R2402026" />
												<input id="repay_0_repay_type" name="repay_0_repay_type" type="hidden" value="normal" />
												<input id="repay_0_repay_installment" name="repay_0_repay_installment" type="hidden" value="1" />
												<input id="repay_0_repay_amount" name="repay_0_repay_amount" type="hidden" value="2600.00" />
												<input id="repay_0_repay_overdue_interest" name="repay_0_repay_overdue_interest" type="hidden" value="0.00" />
												<input id="repay_0_repay_overdue_interest_upper" name="repay_0_repay_overdue_interest_upper" type="hidden" value="0.00" />
												<input id="repay_0_repay_handling_fee" name="repay_0_repay_handling_fee" type="hidden" value="0.00" />
												<input id="repay_0_received_amount" name="repay_0_received_amount" type="hidden" value="2600.00" />
												<input id="repay_0_payment_remark" name="repay_0_payment_remark" type="hidden" value="" />
												<input id="repay_0_migrate_lock" name="repay_0_migrate_lock" type="hidden" value="" />
											</td>
										</tr>
																																								<tr class="repay_row" id="repay_1" onClick="select_repay($(this));" style="cursor: pointer; ">
											<td style="text-align: center;"><input class="select_choice" name="select_choice" type="radio" value="1" /></td>
											<td>2024-02-12</td>
											<td>R2402044</td>
											<td>還款</td>
											<td style="text-align: center;">1</td>
											...
											<td style="text-align: center;"><button class="btn btn-small" type="button" onClick="if ($.browser.msie) { window.event.cancelBubble = true; } else { event.stopPropagation(); } open_repay_detail('view', '418');">詳細</button>
												<input id="repay_1_void" name="repay_1_void" type="hidden" value="" />
												<input id="repay_1_void2" name="repay_1_void2" type="hidden" value="" />
												<input id="repay_1_repay_id" name="repay_1_repay_id" type="hidden" value="418" />
												<input id="repay_1_autopay_id" name="repay_1_autopay_id" type="hidden" value="0" />
												<input id="repay_1_autopay_no" name="repay_1_autopay_no" type="hidden" value="" />
												<input id="repay_1_autopay_status" name="repay_1_autopay_status" type="hidden" value="" />
												<input id="repay_1_repay_date" name="repay_1_repay_date" type="hidden" value="2024-02-12" />
												<input id="repay_1_repay_no" name="repay_1_repay_no" type="hidden" value="R2402044" />
												<input id="repay_1_repay_type" name="repay_1_repay_type" type="hidden" value="normal" />
												<input id="repay_1_repay_installment" name="repay_1_repay_installment" type="hidden" value="1" />
												<input id="repay_1_repay_amount" name="repay_1_repay_amount" type="hidden" value="22.00" />
												<input id="repay_1_repay_overdue_interest" name="repay_1_repay_overdue_interest" type="hidden" value="0.00" />
												<input id="repay_1_repay_overdue_interest_upper" name="repay_1_repay_overdue_interest_upper" type="hidden" value="0.00" />
												<input id="repay_1_repay_handling_fee" name="repay_1_repay_handling_fee" type="hidden" value="0.00" />
												<input id="repay_1_received_amount" name="repay_1_received_amount" type="hidden" value="40.00" />
												<input id="repay_1_payment_remark" name="repay_1_payment_remark" type="hidden" value="" />
												<input id="repay_1_migrate_lock" name="repay_1_migrate_lock" type="hidden" value="" />
											</td>
										</tr>
																																								<tr class="repay_row" id="repay_2" onClick="select_repay($(this));" style="cursor: pointer; ">
											<td style="text-align: center;"><input class="select_choice" name="select_choice" type="radio" value="2" /></td>
											<td>2024-02-24</td>
											<td>R2402090</td>
											<td>還款</td>
											<td style="text-align: center;">2</td>
											<td style="text-align: right;">2,622.00</td>
											<td style="text-align: right;">0.00																							</td>
											<td style="text-align: right;">0.00</td>
											<td style="text-align: right;">2,620.00</td>
											<td style="text-align: right;">-2.00</td>
											<td style="text-align: center;">
																							</td>
											<td style="text-align: center;"><button class="btn btn-small" type="button" onClick="if ($.browser.msie) { window.event.cancelBubble = true; } else { event.stopPropagation(); } open_repay_detail('view', '464');">詳細</button>
												<input id="repay_2_void" name="repay_2_void" type="hidden" value="" />
												<input id="repay_2_void2" name="repay_2_void2" type="hidden" value="" />
												<input id="repay_2_repay_id" name="repay_2_repay_id" type="hidden" value="464" />
												<input id="repay_2_autopay_id" name="repay_2_autopay_id" type="hidden" value="0" />
												<input id="repay_2_autopay_no" name="repay_2_autopay_no" type="hidden" value="" />
												<input id="repay_2_autopay_status" name="repay_2_autopay_status" type="hidden" value="" />
												<input id="repay_2_repay_date" name="repay_2_repay_date" type="hidden" value="2024-02-24" />
												<input id="repay_2_repay_no" name="repay_2_repay_no" type="hidden" value="R2402090" />
												<input id="repay_2_repay_type" name="repay_2_repay_type" type="hidden" value="normal" />
												<input id="repay_2_repay_installment" name="repay_2_repay_installment" type="hidden" value="2" />
												<input id="repay_2_repay_amount" name="repay_2_repay_amount" type="hidden" value="2622.00" />
												<input id="repay_2_repay_overdue_interest" name="repay_2_repay_overdue_interest" type="hidden" value="0.00" />
												<input id="repay_2_repay_overdue_interest_upper" name="repay_2_repay_overdue_interest_upper" type="hidden" value="0.00" />
												<input id="repay_2_repay_handling_fee" name="repay_2_repay_handling_fee" type="hidden" value="0.00" />
												<input id="repay_2_received_amount" name="repay_2_received_amount" type="hidden" value="2620.00" />
												<input id="repay_2_payment_remark" name="repay_2_payment_remark" type="hidden" value="" />
												<input id="repay_2_migrate_lock" name="repay_2_migrate_lock" type="hidden" value="" />
											</td>
										</tr>
																																								<tr class="repay_row" id="repay_3" onClick="select_repay($(this));" style="cursor: pointer; ">
											<td style="text-align: center;"><input class="select_choice" name="select_choice" type="radio" value="3" /></td>
											<td>2024-03-11</td>
											<td>R2403027</td>
											<td>還款</td>
											...
											<td style="text-align: center;"><button class="btn btn-small" type="button" onClick="if ($.browser.msie) { window.event.cancelBubble = true; } else { event.stopPropagation(); } open_repay_detail('view', '508');">詳細</button>
												<input id="repay_3_void" name="repay_3_void" type="hidden" value="" />
												<input id="repay_3_void2" name="repay_3_void2" type="hidden" value="" />
												<input id="repay_3_repay_id" name="repay_3_repay_id" type="hidden" value="508" />
												<input id="repay_3_autopay_id" name="repay_3_autopay_id" type="hidden" value="0" />
												<input id="repay_3_autopay_no" name="repay_3_autopay_no" type="hidden" value="" />
												<input id="repay_3_autopay_status" name="repay_3_autopay_status" type="hidden" value="" />
												<input id="repay_3_repay_date" name="repay_3_repay_date" type="hidden" value="2024-03-11" />
												<input id="repay_3_repay_no" name="repay_3_repay_no" type="hidden" value="R2403027" />
												<input id="repay_3_repay_type" name="repay_3_repay_type" type="hidden" value="normal" />
												<input id="repay_3_repay_installment" name="repay_3_repay_installment" type="hidden" value="3" />
												<input id="repay_3_repay_amount" name="repay_3_repay_amount" type="hidden" value="2622.00" />
												<input id="repay_3_repay_overdue_interest" name="repay_3_repay_overdue_interest" type="hidden" value="0.00" />
												<input id="repay_3_repay_overdue_interest_upper" name="repay_3_repay_overdue_interest_upper" type="hidden" value="0.00" />
												<input id="repay_3_repay_handling_fee" name="repay_3_repay_handling_fee" type="hidden" value="0.00" />
												<input id="repay_3_received_amount" name="repay_3_received_amount" type="hidden" value="2620.00" />
												<input id="repay_3_payment_remark" name="repay_3_payment_remark" type="hidden" value="" />
												<input id="repay_3_migrate_lock" name="repay_3_migrate_lock" type="hidden" value="" />
											</td>
										</tr>
																																								<tr class="repay_row" id="repay_4" onClick="select_repay($(this));" style="cursor: pointer; ">
											<td style="text-align: center;"><input class="select_choice" name="select_choice" type="radio" value="4" /></td>
											<td>2024-03-26</td>
											<td>R2403084</td>
											<td>還款</td>
											...
											<td style="text-align: center;"><button class="btn btn-small" type="button" onClick="if ($.browser.msie) { window.event.cancelBubble = true; } else { event.stopPropagation(); } open_repay_detail('view', '567');">詳細</button>
												<input id="repay_4_void" name="repay_4_void" type="hidden" value="" />
												<input id="repay_4_void2" name="repay_4_void2" type="hidden" value="" />
												<input id="repay_4_repay_id" name="repay_4_repay_id" type="hidden" value="567" />
												<input id="repay_4_autopay_id" name="repay_4_autopay_id" type="hidden" value="0" />
												<input id="repay_4_autopay_no" name="repay_4_autopay_no" type="hidden" value="" />
												<input id="repay_4_autopay_status" name="repay_4_autopay_status" type="hidden" value="" />
												<input id="repay_4_repay_date" name="repay_4_repay_date" type="hidden" value="2024-03-26" />
												<input id="repay_4_repay_no" name="repay_4_repay_no" type="hidden" value="R2403084" />
												<input id="repay_4_repay_type" name="repay_4_repay_type" type="hidden" value="normal" />
												<input id="repay_4_repay_installment" name="repay_4_repay_installment" type="hidden" value="4" />
												<input id="repay_4_repay_amount" name="repay_4_repay_amount" type="hidden" value="2622.00" />
												<input id="repay_4_repay_overdue_interest" name="repay_4_repay_overdue_interest" type="hidden" value="0.00" />
												<input id="repay_4_repay_overdue_interest_upper" name="repay_4_repay_overdue_interest_upper" type="hidden" value="0.00" />
												<input id="repay_4_repay_handling_fee" name="repay_4_repay_handling_fee" type="hidden" value="0.00" />
												<input id="repay_4_received_amount" name="repay_4_received_amount" type="hidden" value="2620.00" />
												<input id="repay_4_payment_remark" name="repay_4_payment_remark" type="hidden" value="" />
												<input id="repay_4_migrate_lock" name="repay_4_migrate_lock" type="hidden" value="" />
											</td>
										</tr>
																																							</tbody>
									...
									<tfoot>
										<tr>
											<td style="text-align: right;" colspan="5"><strong>總金額:</strong></td>
											<td style="text-align: right;">10,488.00</td>
											<td style="text-align: right;">0.00																							</td>
											<td style="text-align: right;">0.00</td>
								...
								<input id="repay_count" name="repay_count" type="hidden" value="5" />
							</div>
							
							<div class="tab-pane" id="tab-repay-condition-information">
																
								<div class="row-fluid">
									<div class="span12">
										<fieldset>
											<legend style="margin-bottom: 8px;">還款狀況</legend>
											
											<div class="row-fluid">
												<div class="span6">
													<div class="control-group" id="repaid_installment-control">
														<label class="control-label" for="repaid_installment">已供 / 總期數</label>
														<div class="controls">
															<input class="" id="installment_placeholder" name="installment_placeholder" type="text" value="4 / 4" style="width: 206px;" readonly />
															<input type="hidden" id="repaid_installment" name="repaid_installment" value="4" />
															<input type="hidden" id="total_installment" name="total_installment" value="4" />
															<span class="help-inline"></span>
														</div>
													</div><br />
													
													<div class="control-group hide" id="latest_repay_date-control">
														<label class="control-label" for="latest_repay_date">最後還款日期</label>
														<div class="controls">
															<input class="" id="latest_repay_date" name="latest_repay_date" type="text" value="2024-03-26" style="width: 206px;" readonly />
															<span class="help-inline"></span>
														</div>
													</div>
													
													<div class="control-group hide" id="next_repay_date-control">
														<label class="control-label" for="next_repay_date">下期還款日期</label>
														<div class="controls">
															<input class="" id="next_repay_date" name="next_repay_date" type="text" value="" style="width: 206px;" readonly />
															<span class="help-inline"></span>
														</div>
													</div>
													
													<div class="control-group" id="principal-control">
														<label class="control-label" for="principal">本金</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="principal" name="principal" type="text" value="10,000.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div>
													<div class="control-group" id="repaid_principal-control">
														<label class="control-label" for="repaid_principal">已供本金</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="repaid_principal" name="repaid_principal" type="text" value="10,000.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div>
													<div class="control-group" id="principal_balance-control">
														<label class="control-label" for="principal_balance">尚欠本金</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="principal_balance" name="principal_balance" type="text" value="0.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div><br />
													
													<div class="control-group" id="interest-control">
														<label class="control-label" for="interest">利息</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="interest" name="interest" type="text" value="488.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div>
													<div class="control-group" id="repaid_interest-control">
														<label class="control-label" for="repaid_interest">已供利息</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="repaid_interest" name="repaid_interest" type="text" value="488.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div>
													<div class="control-group" id="interest_balance-control">
														<label class="control-label" for="interest_balance">尚欠利息</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="interest_balance" name="interest_balance" type="text" value="0.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div><br />
												</div>
												...
												<div class="span6">
													<div class="control-group" id="interest_method-control">
														...
														<div class="controls">
															<input class="" id="interest_method_label" name="interest_method_label" type="text" value="息隨本減" style="width: 206px;" readonly />
															<input id="interest_method_hd" name="interest_method" type="hidden" value="int_prin" />
														</div>
													</div>
													
													<div class="control-group" id="flat_rate-control">
														<label class="control-label" for="flat_rate">期平息</label>
														<div class="controls">
															<div class="input-append">
																<input class="number rate" data-decimal-place="4" id="flat_rate" name="flat_rate" type="text" value="1.2200" style="width: 179px;" readonly /><span class="add-on">%</span>
															</div>
															<span class="help-inline"></span>
														</div>
													</div>
													
													<div class="control-group" id="interest_rate-control">
														<label class="control-label" for="interest_rate">年利率</label>
														<div class="controls">
															<div class="input-append">
																<input class="number rate" data-decimal-place="3" id="interest_rate" name="interest_rate" type="text" value="47.000" style="width: 179px;" readonly /><span class="add-on">%</span>
															</div>
															<span class="help-inline"></span>
														</div>
													</div><br />
													
													<div class="control-group" id="repaid_overdue_interest-control">
														<label class="control-label" for="repaid_overdue_interest">已收過期利息</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="repaid_overdue_interest" name="repaid_overdue_interest" type="text" value="0.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div>
													
													<div class="control-group" id="repaid_handling_fee-control">
														<label class="control-label" for="repaid_handling_fee">已收行政費</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="repaid_handling_fee" name="repaid_handling_fee" type="text" value="0.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div>
													
													<div class="control-group" id="repaid_temp_amount-control">
														<label class="control-label" for="repaid_temp_amount">暫存總金額</label>
														<div class="controls">
															<div class="input-prepend">
																<span class="add-on">$</span><input class="" id="repaid_temp_amount" name="repaid_temp_amount" type="text" value="12.00" style="width: 179px;" readonly />
															</div>
														</div>
													</div><br />
													
													<div class="control-group" id="repay_schedule-control">
														<label class="control-label" for="repay_schedule">還款表</label>
														<div class="controls">
															<button class="btn" type="button"  onClick="new_window('print_docs_loan_integrated.php?type=view&id=125&init=repay_table', 750, 800, 'print_docs_loan_approval');"  title="列印"><i class="icon-print"></i> 列印</button>
															<span class="help-inline"></span>
														</div>
													</div>
													
													<div class="control-group" id="outstanding_details-control">
														<label class="control-label" for="outstanding_details">客戶還款表欠款詳細</label>
														<div class="controls">
															<button class="btn" type="button" onClick="new_window('report_outstanding_details_print.php?loan_id=125', screen.width, screen.height, 'report_outstanding_details_print');" title="列印"><i class="icon-print"></i> 列印</button>
															<span class="help-inline"></span>
														</div>
													</div>
												</div>
	...
	<div class="modal hide" id="void-modal">
		...
		<div class="modal-body">
			<p>被 Void 的資料將無法還原，請確認是否 Void？</p>
		</div>
		
		<div class="modal-footer">
			<button class="btn" type="button" data-dismiss="modal">取消</button>
			<button class="btn btn-primary" id="void-button" type="button" data-loading-text="載入中.." onClick="if (no_submit) return; force_submit = true; $('#main-form').attr('action', '?type=void').submit();">確定</button>
		</div>
	</div><!--/#void-modal-->
	
	
	<div class="modal hide" id="void2-modal">
		<div class="modal-header">
			<a class="close" data-dismiss="modal">&times;</a>
			<h3>確認退款/票</h3>
		</div>
		...
		<div class="modal-body">
			<p>被退款/票的資料將無法還原，請確認是否退款/票？</p><br />
			
			<form class="form-horizontal" id="void2-form" name="void2-form" method="post">
				<div class="control-group" id="void2_date-control" style="margin-left: 40px;">
					<label class="control-label" for="void2_date" style="text-align: right;">退款/票日期</label>
					<div class="controls">
						<input class="required mydate calendar" id="void2_date" name="void2_date" type="text" maxlength="10" value="" placeholder="YYYY-MM-DD" style="width: 206px;" />
						<span class="help-inline"></span>
					</div>
				</div>
			</form>
		...
		<div class="modal-footer">
			<button class="btn" type="button" data-dismiss="modal">取消</button>
			<button class="btn btn-primary" id="void2-button" type="button" data-loading-text="載入中.." onClick="if (no_submit) return; if (!check_active_period(this, $('#void2_date').val())) return; $('#void2-form').attr('action', '?type=void2').submit();">確定</button>
		</div>
	</div><!--/#void2-modal-->
